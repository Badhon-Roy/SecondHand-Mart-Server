...
const fetchPurchaseHistory = async (userId: string) => {
    const result = await OrderModel.find({ "buyerID": userId }).populate('itemID').populate('sellerID');
    return result;
};
...
const fetchSalesHistory = async (userId: string) => {
    const result = await OrderModel.find({ "sellerID": userId }).populate("itemID").populate('buyerID');
    return result;
};
...
const updateOrderStatusFromDB = async (
    ...
    try {
        ...
        const order = await OrderModel.findOne({ sellerID: sellerObjectId, _id: orderObjectId });
